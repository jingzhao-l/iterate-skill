...
import type { ToolErrorInfo, ToolExecution, PreToolDecision } from '@deepseek-ai/dsh-tools'
...
export function gateDecision(exec: ToolExecution): PreToolDecision {
  ...
  let signal: Readonly<AbortSignal> | undefined
  try {
    signal = exec.signal
  } catch {
    signal = undefined
