...
- [🧭 双模式架构](#-双模式架构)
- [📖 日常使用](#-日常使用)
- [✨ Iterate 特性](#-iterate-特性)
- [🔧 七个 iterate 工具](#-七个-iterate-工具)
- [🧭 架构](#-架构)
- [📦 安装与测试](#-安装与测试)
- [🚑 故障排查](#-故障排查)
- [📄 许可与免责声明](#-许可与免责声明)
...
```bash
export ANTHROPIC_API_KEY=your_key   # 或用 OpenAI 兼容供应商，见下文
```
...
/iterate run           # normal 模式：评审 → 修复 → 验证 → 循环
/iterate status        # 配置 + onboarding 状态 + 漂移检查
/iterate log           # 查看决策日志尾部
```

也可以纯 CLI 跑一个最小无头闭环：

```bash
ih iterate onboard     # 模型驱动项目扫描 → ITERATE.md + 配置 + manifest 指纹
ih iterate review      # dry-run：反复评审直到收敛
ih iterate run         # 自治修复闭环
```

> 任何一个项目的第一条命令都是 `ih iterate onboard`（不想要模型调用时可用
> 纯检测路径 `ih iterate onboard --no-ai`）。它生成项目知识库——与 skill 生态
> 完全一致的 `ITERATE.md` + `iterate.config.yaml`。下面所有内容都假设这步已完成。
...
任意 OpenAI 兼容供应商都可用。管理入口：

```bash
ih provider list        # 查看可用供应商及其默认模型
ih provider use <name>  # 激活 profile（交互式重新输入 Key）
ih provider edit <name> # 修正 base_url / api_format / default_model / auth_source
```
...
```bash
ih iterate onboard            # 模型驱动扫描 → ITERATE.md + 配置 + manifest 指纹
...
Onboarding 说明：`ih iterate onboard` 先做模型凭证门禁，然后让模型用自己的
只读工具扫描项目（manifest、2-3 层目录树、specs/tests/CI、README——绝不碰
`.env`/密钥），按字节精确的「AI 维护区 / 用户维护区」标记写出 `ITERATE.md`；
harness 代码负责校验标记、采集 manifest SHA-256 指纹并写 `iterate.config.yaml`——
不可信的模型输出永远进不了受信配置结构。两个产物与 skill 侧 onboarding
字节兼容（同标记、同 `onboarding.fingerprints` schema），两个生态互相可读。
之后每次循环 kickoff 都会把 `ITERATE.md` 知识库注入系统提示，manifest 漂移
（升依赖、换技术栈）会在评审前给出非阻塞警告。
...
> TUI 等价入口：`/iterate` 斜杠命令（status / review / run / log / config /
> validate）以不同入口提供同样的闭环。
