# Contributing to IterateHarness

IterateHarness is an open-source agent harness focused on clarity, hackability, and compatibility with Claude-style workflows.
...
```bash
git clone https://github.com/HKUDS/IterateHarness.git
cd IterateHarness
uv sync --extra dev
```
...
- If you are improving type coverage, feel free to run `uv run mypy src/iterate_harness`, but it is not yet a required green check for the whole repo.

## Documentation and community contributions

Issue [#7](https://github.com/HKUDS/IterateHarness/issues/7) surfaced several high-value docs needs. Useful contributions in that area include:
