...
async def test_grep_tool_returns_timeout_error(monkeypatch, tmp_path: Path):
    tool = GrepTool()
    monkeypatch.setattr("iterate_harness.tools.grep_tool.shutil.which", lambda _: "/usr/bin/rg")
    ...
    monkeypatch.setattr(
        "iterate_harness.tools.grep_tool.asyncio.create_subprocess_exec",
...
async def test_grep_tool_python_fallback_reports_invalid_regex(monkeypatch, tmp_path: Path):
    tool = GrepTool()
    monkeypatch.setattr("iterate_harness.tools.grep_tool.shutil.which", lambda _: None)
    file_path = tmp_path / "notes.txt"
    file_path.write_text("hello\n", encoding="utf-8")
