...
from iterate_harness.auth.storage import ExternalAuthBinding, load_external_binding, store_external_binding
from iterate_harness.cli import app
...
def test_load_claude_external_credential(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("iterate_harness.auth.external.platform.system", lambda: "Linux")
...
def test_default_claude_binding_uses_keychain_on_macos(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("iterate_harness.auth.external.platform.system", lambda: "Darwin")
...
def test_load_claude_external_credential_from_keychain(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("iterate_harness.auth.external.subprocess.check_output", _fake_check_output)
...
def test_settings_resolve_auth_refreshes_expired_external_binding(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(
        "iterate_harness.auth.external.refresh_claude_oauth_credential",
...
def test_cli_codex_login_binds_without_switching(monkeypatch, tmp_path: Path):
    ...
    assert "Use `ih provider use codex` to activate it." in result.stdout
...
def test_cli_claude_login_binds_without_switching(monkeypatch, tmp_path: Path):
    ...
    assert "Use `ih provider use claude-subscription` to activate it." in result.stdout
...
def test_load_claude_external_credential_refreshes_expired_keychain(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("iterate_harness.auth.external.subprocess.check_output", _fake_check_output)
    monkeypatch.setattr("iterate_harness.auth.external.subprocess.run", _fake_run)
    monkeypatch.setattr(
        "iterate_harness.auth.external.refresh_claude_oauth_credential",
        lambda refresh_token: {
            "access_token": "fresh-token",
...
def test_refresh_claude_oauth_credential(monkeypatch):
    ...
    monkeypatch.setattr("iterate_harness.auth.external.urllib.request.urlopen", _fake_urlopen)
    monkeypatch.setattr("iterate_harness.auth.external.time.time", lambda: 1000)
...
def test_refresh_claude_oauth_credential_reports_invalid_grant(monkeypatch):
    ...
    monkeypatch.setattr(
        "iterate_harness.auth.external.urllib.request.urlopen",
...
def test_get_claude_code_version_uses_fallback(monkeypatch):
    ...
    monkeypatch.setattr(
        "iterate_harness.auth.external.subprocess.run",
        lambda *args, **kwargs: _Result(),
    )
    monkeypatch.setattr("iterate_harness.auth.external._claude_code_version_cache", None)
