...
from iterate_harness.output_styles.loader import load_output_styles


def test_builtin_output_styles_include_compact(monkeypatch, tmp_path):
    ...
    assert {"default", "minimal", "compact"}.issubset(builtin_names)
...
def test_custom_output_style_is_loaded(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("ITERATE_CONFIG_DIR", str(config_dir))
