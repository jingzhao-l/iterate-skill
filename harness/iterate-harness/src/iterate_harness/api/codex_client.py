...
from iterate_harness.api.errors import AuthenticationFailure, IterateHarnessApiError, RateLimitFailure, RequestFailure
from iterate_harness.api.usage import UsageSnapshot
from iterate_harness.engine.messages import ConversationMessage, ImageBlock, TextBlock, ToolResultBlock, ToolUseBlock
...
def _build_codex_headers(token: str, *, session_id: str | None = None) -> dict[str, str]:
    ...
    headers = {
        ...
        "originator": "iterate_harness",
        "User-Agent": f"iterate_harness ({platform.system().lower()} {platform.machine() or 'unknown'})",
...
def _translate_status_error(status_code: int, message: str) -> IterateHarnessApiError:
...
class CodexApiClient:
    ...
    async def _stream_once(self, request: ApiMessageRequest) -> AsyncIterator[ApiStreamEvent]:
        body: dict[str, Any] = {
            ...
            "store": False,
            "stream": True,
            "instructions": request.system_prompt or "You are IterateHarness.",
    ...
    @staticmethod
    def _translate_error(exc: Exception) -> IterateHarnessApiError:
        if isinstance(exc, IterateHarnessApiError):
            return exc
        if isinstance(exc, httpx.HTTPStatusError):
