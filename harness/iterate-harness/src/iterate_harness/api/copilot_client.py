"""GitHub Copilot API client for IterateHarness.
...
class CopilotClient:
    ...
    github_token:
        ...
        token is loaded from ``~/.iterate-harness/copilot_auth.json``.
    enterprise_url:
        ...
        *,
        ...
        if not token:
            raise AuthenticationFailure(
                "No GitHub Copilot token found. Run 'ih auth copilot-login' first."
            )
    ...
    async def stream_message(self, request: ApiMessageRequest) -> AsyncIterator[ApiStreamEvent]:
        ...
        the IterateHarness query engine.
