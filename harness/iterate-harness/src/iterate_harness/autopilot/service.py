...
from iterate_harness.engine.stream_events import AssistantTextDelta, AssistantTurnComplete, ErrorEvent
...
class RepoAutopilotStore:
        ...
        for label, path in discovered[:limit]:
            ...
            card, _ = self.enqueue_card(
                ...
                body=(
                    ...
                    "Review whether this should be aligned, adapted, or ignored for IterateHarness."
                ),
    ...
    def install_default_cron(self) -> list[str]:
        from iterate_harness.services.cron import upsert_cron_job

        jobs = [
            {
                "name": "autopilot.scan",
                "schedule": "*/30 * * * *",
                "command": f"ih autopilot scan all --cwd {self._cwd}",
                "cwd": str(self._cwd),
            },
            {
                "name": "autopilot.tick",
                "schedule": "0 */2 * * *",
                "command": f"ih autopilot tick --cwd {self._cwd}",
                "cwd": str(self._cwd),
            },
        ]
        for job in jobs:
    ...
    def _comment_started(self, card: RepoTaskCard, attempt_count: int) -> str:
        return _bilingual_lines(
            f"IterateHarness autopilot 已开始处理 `{card.id}`，当前第 {attempt_count} 轮执行。",
            f"IterateHarness autopilot started processing `{card.id}`. Attempt {attempt_count} is now running.",
        )
    ...
    def _render_dashboard_html(self, snapshot: dict[str, Any]) -> str:
        ...
        """
        repo_name = escape(_safe_text(snapshot.get("repo_name")) or "IterateHarness")
