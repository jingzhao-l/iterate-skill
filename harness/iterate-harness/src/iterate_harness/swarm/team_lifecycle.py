"""Persistent team lifecycle management for IterateHarness swarms.

Teams are stored as JSON files on disk:
    ~/.iterate-harness/teams/<name>/team.json
...
class TeamLifecycleManager:
    ...
    Persists team metadata to ``~/.iterate-harness/teams/<name>/team.json``.
    ...
    def list_teams(self) -> list[TeamFile]:
        """Return all teams found in ``~/.iterate-harness/teams/``, sorted by name."""
        base = Path.home() / ".iterate-harness" / "teams"
        if not base.exists():
            return []
