"""Permission sync protocol for leader-worker coordination in IterateHarness swarms.
...
Paths:
    ~/.iterate-harness/teams/<teamName>/permissions/pending/<id>.json
    ~/.iterate-harness/teams/<teamName>/permissions/resolved/<id>.json
"""
...
from iterate_harness.swarm.mailbox import (
    MailboxMessage,
    TeammateMailbox,
...
def get_permission_dir(team_name: str) -> Path:
    """Return ~/.iterate-harness/teams/{teamName}/permissions/"""
...
async def get_leader_name(team_name: str | None = None) -> str | None:
    ...
    """
    from iterate_harness.swarm.team_lifecycle import read_team_file_async
...
async def handle_permission_request(
    ...
    Args:
        ...
        checker: An already-configured :class:`~iterate_harness.permissions.checker.PermissionChecker`.
