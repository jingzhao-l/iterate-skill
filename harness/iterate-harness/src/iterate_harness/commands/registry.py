...
from iterate_harness.config.settings import Settings, display_model_setting, load_settings, save_settings
...
from iterate_harness.memory import (
    add_memory_entry,
...
if TYPE_CHECKING:
    ...
    from iterate_harness.state import AppStateStore
...
def create_default_command_registry(
    ...
    async def _init_handler(args: str, context: CommandContext) -> CommandResult:
        ...
        if not claudemd.exists():
            claudemd.write_text(
                ...
                "- Use IterateHarness tools deliberately.\n"
        ...
        for relative, content in (
            (
                ...
                "# Project IterateHarness Config\n\nThis directory stores project-specific IterateHarness state.\n",
            ),
            (
        ...
        if not created:
            return CommandResult(message="Project already initialized for IterateHarness.")
    ...
    async def _login_handler(args: str, context: CommandContext) -> CommandResult:
        ...
        return CommandResult(message="Stored API key in ~/.iterate-harness/settings.json")
    ...
    async def _theme_handler(args: str, context: CommandContext) -> CommandResult:
        from iterate_harness.themes import list_themes, load_theme
    ...
    async def _keybindings_handler(_: str, context: CommandContext) -> CommandResult:
        from iterate_harness.keybindings import get_keybindings_path, load_keybindings
    ...
    async def _privacy_settings_handler(_: str, context: CommandContext) -> CommandResult:
        ...
        lines = [
            ...
            "- storage: local files under ~/.iterate-harness and project .iterate-harness",
        ]
        return CommandResult(message="\n".join(lines))
    ...
    async def _release_notes_handler(_: str, context: CommandContext) -> CommandResult:
        ...
        return CommandResult(
            message=(
                "# Release Notes\n\n"
                "- React TUI is now the default `ih` interface.\n"
    ...
    registry.register(
        SlashCommand("exit", "Exit IterateHarness", _exit_handler, aliases=("quit",))
    )
    registry.register(SlashCommand("clear", "Clear conversation history", _clear_handler))
    registry.register(SlashCommand("version", "Show the installed IterateHarness version", _version_handler))
    ...
    registry.register(SlashCommand("init", "Initialize project IterateHarness files", _init_handler))
    registry.register(
        SlashCommand(
            "iterate",
    ...
    registry.register(SlashCommand("release-notes", "Show recent IterateHarness release notes", _release_notes_handler))
...
def _memory_backend_for_context(context: CommandContext) -> MemoryCommandBackend:
    ...
    return MemoryCommandBackend(
        label="IterateHarness project memory",
