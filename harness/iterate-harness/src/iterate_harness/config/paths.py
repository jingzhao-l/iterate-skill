"""Path resolution for IterateHarness configuration and data directories.

Follows XDG-like conventions with ~/.iterate-harness/ as the default base directory.
"""
...
_DEFAULT_BASE_DIR = ".iterate-harness"
...
def get_config_file_path() -> Path:
    """Return the path to the main settings file (~/.iterate-harness/settings.json)."""
...
def get_project_config_dir(cwd: str | Path) -> Path:
    """Return the per-project .iterate-harness directory."""
    project_dir = Path(cwd).resolve() / ".iterate-harness"
