...
def render_config_text(config: dict[str, object]) -> str:
    ...
    header = (
        "# iterate configuration — generated by `ih iterate init`\n"
        "# Docs: validation.commands entries are matched EXACTLY (whitelist).\n"
    )
    return header + str(yaml.safe_dump(config, sort_keys=False, allow_unicode=True))
...
def parse_dimension_selection(raw: str, offered: list[str]) -> list[str] | None:
    ...
    for token in text.replace(",", " ").split():
        if token.isdigit():
            ...
            candidate = name_to_dim.get(token.lower().replace("_", "-"))
