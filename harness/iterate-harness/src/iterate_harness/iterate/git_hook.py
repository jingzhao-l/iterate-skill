...
``ih iterate hook install`` writes a managed ``.git/hooks/pre-commit`` that
...
- the ``ih`` binary is resolved to an absolute path at install time because
...
# Managed by `ih iterate hook`. Re-run that command to update; remove with
# `ih iterate hook uninstall`. Skip once with ITERATE_SKIP_HOOK=1 or
...
def hook_status(cwd: str | Path) -> dict[str, Any]:
    """Describe the hook state for `ih iterate hook status`."""
    try:
        target = hook_path(cwd)
